...
The **YOUR-RUN, PEND-RES, PEND-OTHR**, and **YOUR-TOTL** columns shows the counts of the running, resource pending, other pending, and total job count of the current user, respectively. If these four columns have same values, these same values of that four columns will be shown at COMMON VALUES as four single values.

The **MIN NODE** and **MAX NODE** columns show the permitted minimum and maximum node counts of the
 jobs which can be submitted to the partition.

The **MAXCPU/NODE** column shows the permitted maximum core counts of the single node in
 the partition.
...
The **DEFAULT JOB-TIME** column shows the default time limit of the job which submitted to the
 partition without a time limit. If the DEFAULT JOB-TIME limits are not set, or set
...
The **MAXIMUM JOB-TIME** column shows the maximum time limit of the job which submitted to the
...
The **CORES /NODE** column shows the core count of the node with the lowest core count in the
...
The **QOS NAME** column shows the default qos limits the job which submitted to the partition.
 If the QOS NAME of the partition are not set for all partitions in your cluster, QOS NAME
 column will not be shown, except -l parameter was given.

The **GRES (COUNT)** column shows the generic resources of the nodes in the partition, and (in
 parentheses) the total number of nodes in that partition containing that GRES. The GRES (COUNT)
 column will not be shown, except -l or -g parameter was given.

If the partition's **QOS NAME, MIN NODES, MAX NODES, MAXCPU/NODE, DEFMEM GB/CPU|NODE,
 MAXMEM GB/CPU|NODE, DEFAULT JOB-TIME**, and **MAXIMUM JOB-TIME** limits are not set for the
...
**-g** the output shows each GRES (gpu, mic etc.) defined in that partition
  and (in parentheses) the total number of nodes in that partition
...
**-J** the output does not show the info about the user's jobs.

**-f** the output shows each FEATURES defined in that partition and (in parentheses)
  the total number of nodes in that partition containing that FEATURES.

**-p PARTITION_LIST**
the output shows only the partitions which given with comma-separated PARTITION_LIST.

**-l** all possible columns will be shown, except the federated clusters column.
...
When STATEMENT feature is on, the spart looks for the statements files. To open the STATEMENT feature, the **SPART_SHOW_STATEMENT** macro must be defined that can be achieved when you uncomment line 24 in spart.h file. If the spart finds any statement file, it shows the content of the files. If the spart can not find a particular statement file, simply ignore it without the notification. Therefore, there is no need to recompile to reshow or cancel statements. You can edit or remove statement files after compiling. If you want to show again, just write new file(s).

The spart also can show the statements using different font styles and background colors. The style of the cluster's statement and the partitions' statements can be set differently.
...
The spart requires, the slurm configured to give permission for reading other users job info,
...
If you notice a bug of the spart, please report using the issue page of the GitHub site. Thanks.
